...
int main(int argc, char **argv) {
  ...
  msd::channel<InputFrame> input_channel{10};
  msd::channel<OutputFrame> output_channel;

  vector<thread> processor_threads;
  vector<unique_ptr<ImageProcessor>> processors;

  for (int i = 0; i < 2; ++i) {
    processors.push_back(
        make_unique<ImageProcessor>(input_channel, output_channel, 1 + i));
    processor_threads.emplace_back(&ImageProcessor::run,
                                   processors.back().get());
  }

  bool imageReaderRunning = true;
  thread image_reader_thread([&]() {
    ros::init(argc, argv, "gate_pose_estimation",
              ros::init_options::NoSigintHandler);
    ros::NodeHandle nh;

    uint64_t frameIndex = 0;
    // ros::Rate rate(100);
    const auto imageCallback = [&](const sensor_msgs::ImageConstPtr &msg) {
      // std::cout << "Recieved frame: " << frameIndex << std::endl;
      input_channel << InputFrame{cv_bridge::toCvShare(msg, "bgr8")->image,
    ...
    image_transport::Subscriber sub =
        it.subscribe("camera_front/image_raw", 10, imageCallback);

    while (ros::ok() && imageReaderRunning) {
      ros::spinOnce();
      // rate.sleep();
    }

    std::cout << "Finished reading images\n";
    sub.shutdown();
    ros::shutdown();
  });
  ...
  stringstream ss;
  std::cout << "STARTED LISTENING FOR IMAGES\n";
  while (true) {
  ...
  image_reader_thread.join();
  std::cout << "Image reader thread joined\n";
  for (auto &thread : processor_threads) {
    thread.detach();
  }
  std::cout << "All processor threads detached\n";
